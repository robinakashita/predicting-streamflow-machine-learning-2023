...
This repository contains code and notebooks for a hydrologic time-series project focused on infilling missing daily streamflow data using simple linear regression. The analysis uses multi-year records from three stream gauges on the island of Mo'orea, French Polynesia (Tautuapae River, PK 20 Est River, and Vaioro River) to improve inputs for a machine learning model of submarine groundwater discharge.
...
Streamflow records from gauge stations often contain gaps due to equipment failures, shifting channels, or periods of very low flow. These gaps can bias hydrologic models and reduce confidence in long term water balance estimates. In this project, I review and implement several approaches for infilling missing flows, with a focus on simple linear regression between nearby gauges.

I use complete time-series from one gauge as an independent predictor and regress gauges with missing values against this reference station. The resulting regression models are then applied to estimate missing flows and create continuous time-series for each site.
...
### 1. Data preparation
   - Load daily streamflow records from three gauges in Mo'orea.
...
### 2. Simple linear regression
   - Select a nearby gauge with complete data as the independent variable.
   - For Group B, extract:
     - `X`: streamflow at the independent (donor) gauge.
     - `y`: streamflow at the target gauge with gaps.
   - Fit a simple linear regression model:
     \[
     y = aX + b
     \]
   - Apply the fitted model to Group A to estimate missing streamflow values at the target gauge.
...
### 4. Application to multiple gauge pairs
